...
// intentamos leer si ya exite un nombre guardado en el disco del navegador
...
const modalMesage = document.querySelector("#modalMessage");
const modalCancelBtn = document.querySelector("#modalCancelBtn");
const modalConfimBtn = document.querySelector("#modalConfirmBtn");
...
function openCustomModal(title, message, isConfirm = false, onConfirmCallback = null) {
    ...
    modalMesage.textContent = message;

    modalCancelBtn.style.display = isConfirm ? 'block' : 'none' ;
    customModal.classList.add('active');

    const nuevoConfirmBtn = modalConfimBtn.cloneNode(true);
    const nuevoCancelBtn = modalCancelBtn.cloneNode(true);
    modalConfimBtn.parentNode.replaceChild(nuevoConfirmBtn, modalConfirmBtn);
...
async function fetchTasks() {
    try {
        ...
        tasksContainer.innerHTML = `<p class="error">No se puede conectar con el servidor nativo.</p>`;
    }
}
...
function renderTasks(tasks) {
    ...
    tasks.forEach(tasks => {
        ...
        taskCard.className = `task-card ${taskCard.is_completed ? 'completed' : ''}`;
...
async function updateTask(id, title, description, is_completed) {
    try {
        ...
        if (response.ok && json.status === 'success') {
            ...
            openCustomModal('Error de servidor', json.message || 'Error al actualizar en el server', false);
        }
    } catch (error) {
...
async function deleteTask(id, taskAuthor) {
    ...
    openCustomModal(
        ...
        '¿Estas seguro de eliminar esta tarea de la base de datos?',
        true,
        async () => {
            try {
                const response = await fetch('${API_URL}/${id}', {
                ...
                const json = await renponse.json();

                if (reponse.ok && json.status === 'success') {
                    fetchTasks();
                } else {
                    openCustomModal('Error de server', json.message || 'Fallo de autorizacion en el server', false);
                }
            } catch (error) {
                openCustomModal('Error de red', 'Error de red al eliminar la terea.', false);
            }
        }
    );
}
